...
// Schema pour la création d'utilisateur admin (email + password)
...
// Schema pour la création d'utilisateur phone-based (phone + PIN)
export const createUserPhoneSchema = z.object({
  ...
  confirmPin: z
    .string()
    .regex(/^\d{6}$/, "Le PIN doit contenir exactement 6 chiffres"),
  role: z.enum(["vendeur", "livreur", "membre", "equipe"], {
    required_error: "Veuillez sélectionner un rôle",
  }),
...
// Keep backward compat — union type
export const createUserSchema = z.union([createUserAdminSchema, createUserPhoneSchema]);
...
export type CreateUserPhoneFormData = z.infer<typeof createUserPhoneSchema>;
