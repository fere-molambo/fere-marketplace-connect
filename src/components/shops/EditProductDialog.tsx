...
export const EditProductDialog = ({ shopId, product, open, onOpenChange }: EditProductDialogProps) => {
  ...
  const subcategories = categoryId
    ? categories.filter((cat) => cat.parent_id === categoryId)
    : [];
  ...
  useEffect(() => {
    if (product) {
      ...
      setStock(product.quantity_available?.toString() || "0");
      setMinQuantity(product.min_quantity?.toString() || "1");
      setIsActive(product.is_active ?? true);
      setCategoryId(product.category_id || "");
      setSubcategoryId(product.subcategory_id || "");
      setMinAutoPrice(product.min_auto_price?.toString() || "");
      setAutoValidation(product.auto_validation ?? true);
      
      setMainMedia(product.main_media_url || "");
      setHoverMedia(product.hover_media_url || "");
      ...
      if (product.quantity_intervals && Array.isArray(product.quantity_intervals)) {
        setQuantityIntervals(
          product.quantity_intervals.map((interval: any) => ({
            min: interval.min?.toString() || "",
            max: interval.max?.toString() || "",
            price: interval.price?.toString() || ""
          }))
        );
      } else {
        setQuantityIntervals([]);
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <ProductMediaUpload
            ...
            initialMainMedia={product.main_media_url}
            initialHoverMedia={product.hover_media_url}
            initialVideo={product.video_url}
            initialOtherMedia={Array.isArray(product.media_urls) ? product.media_urls : []}
          />
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? "Modification..." : "Modifier le produit"}
            </Button>
          </div>
        </form>
      </DialogContent>
